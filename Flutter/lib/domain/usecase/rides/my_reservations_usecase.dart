import 'package:a_tareqaak/data/models/base/base_model.dart';
import 'package:a_tareqaak/data/models/rides/reservation_data_model.dart';
import 'package:a_tareqaak/domain/entity/rides/rides_no_params_entity.dart';
import 'package:a_tareqaak/domain/repository/rides/i_rides_repository.dart';
import 'package:a_tareqaak/domain/usecase/i_use_case.dart';
import 'package:dartz/dartz.dart';
import 'package:injectable/injectable.dart';
import 'package:a_tareqaak/core/exceptions/app_exception.dart';

@Injectable(
    as: IUseCase<BaseModel<ReservationsListModel>?, RidesNoParamsEntity>)
@Named('MyReservationsUseCase')
class MyReservationsUseCase
    implements
        IUseCase<BaseModel<ReservationsListModel>?, RidesNoParamsEntity> {
  final IRidesRepository _repository;
  MyReservationsUseCase(this._repository);

  @override
  Future<Either<AppException, BaseModel<ReservationsListModel>?>> call(
          RidesNoParamsEntity data) =>
      _repository.myReservations();
}
